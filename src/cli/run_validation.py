...
def append_audit_log(entry: dict, audit_path: str):
...
def run_validation(config: dict, module_config: dict, verify_ledger, verify_ledger_signature_main):
    ...
    private_key_file = config["keys"]["private_key_file"]
    ...
    if first_execution:
        ...
        if not Path(ledger_signature_file).exists():
    ...
    risk_result = dict(module_config["risk_defaults"])
    ...
    core_dossier = {
        ...
        "dossier_type": module_config["dossier_type"],
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "engine_version": module_config["engine_version"],
        "policy_version": module_config["policy_version"],
        "summary_file": summary_file,
        "summary": {},
        "previous_hash": previous_hash,
        "regulatory_context": {
            "rules_version": module_config["rules_version"],
            "rules_hash": module_config["rules_hash"]
        },
        "execution_path": {
            "pipeline_id": module_config["pipeline_id"]
        },
        "risk_decision": {
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        json.dump({"status": "OK", "module": module_config["module_name"]}, f, indent=2, ensure_ascii=False)

    with open(dossier_file, "w", encoding="utf-8") as f:
        json.dump(dossier, f, indent=2, ensure_ascii=False)

    with open(fiscal_file, "w", encoding="utf-8") as f:
        json.dump({"status": "READY", "module": module_config["module_name"]}, f, indent=2, ensure_ascii=False)
    ...
    sign_file_with_openssl(
        ledger_file,
        private_key_file,
        ledger_signature_file
    )
    print(f"Ledger firmato in: {ledger_signature_file}")
    ...
    audit_entry = {
        ...
        "ledger_signature_file": ledger_signature_file,
    ...
    append_audit_log(audit_entry, audit_log_file)
